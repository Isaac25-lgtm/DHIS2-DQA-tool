...
import { Database, HardDriveDownload, KeyRound, Link2, ShieldCheck, Sparkles } from "lucide-react";
      ...
      >
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl bg-brand-surface px-5 py-5">
            <div className="flex items-center gap-3 text-brand-teal">
              <Sparkles size={18} />
              <p className="text-sm font-semibold text-brand-text">AI reporting</p>
            </div>
            <p className="mt-3 text-lg font-bold text-brand-navy">
              {loading
                ? "Loading..."
                : systemInfo?.ai_provider
                  ? `${systemInfo.ai_provider}${systemInfo.ai_model ? ` / ${systemInfo.ai_model}` : ""}`
                  : "Template fallback"}
            </p>
            <p className="mt-2 text-sm text-brand-muted">AI keys remain server-side only. Reports still work without an API key.</p>
          </div>
        </div>
      </Card>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        <Card title="Configuration boundaries" subtitle="Sensitive credentials stay outside frontend API responses and browser storage.">
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-brand-border bg-white px-4 py-4">
              <p className="text-sm font-semibold text-brand-text">Report controls</p>
              <p className="mt-2 text-sm text-brand-muted">
                Official reports are generated by managers or reviewers, reviewed before approval, and exported only through authenticated endpoints.
              </p>
            </div>
            <div className="rounded-2xl border border-brand-border bg-white px-4 py-4">
